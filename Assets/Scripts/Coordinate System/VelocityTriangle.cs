...
public class VelocityTriangle : MonoBehaviour, CannonStateObserver
{
    ...
    public void applyChange(CannonState state){
        ...
        gameObject.transform.eulerAngles = new Vector3(0f, -state.horizontalAngle, 0f);

        Vector3 scale = gameObject.transform.localScale;
        scale.Set((float)Math.Sin(state.verticalAngle * Math.PI/180)*(0.2f * (state.speed - min_vel)/(max_vel - min_vel) + 0.5f), (float)Math.Cos(state.verticalAngle * Math.PI/180)*(0.2f * (state.speed - min_vel)/(max_vel - min_vel) + 0.5f), 0.7f);
